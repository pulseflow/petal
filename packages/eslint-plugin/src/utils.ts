...
export const BLOB_URL = 'https://github.com/pulseflow/petal/blob/main/packages/eslint-plugin/docs/';
...
function RuleCreator(urlCreator: (name: string) => string): <
	TOptions extends readonly unknown[],
	TMessageIds extends string,
>({ meta, name, ...rule }: Readonly<RuleWithMetaAndName<TOptions, TMessageIds>>
) => RuleModule<TOptions, TMessageIds> {
	return <TOptions extends readonly unknown[], TMessageIds extends string>
	({ meta, name, ...rule }: Readonly<RuleWithMetaAndName<TOptions, TMessageIds>>): RuleModule<TOptions, TMessageIds> =>

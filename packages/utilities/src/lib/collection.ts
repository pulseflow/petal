/**
 * Adapted from {@link discordjs/collection https://github.com/discordjs/discord.js/tree/main/packages/collection}
 * @module
 */
...
export class Collection<Key, Value> extends Map<Key, Value> {
	 ...
	 * ```
	 */
	public concat(...collections: Array<ReadonlyCollection<Key, Value>>): Collection<Key, Value> {
	 ...
	 * ```
	 */
	public sort(compareFunction: Comparator<Key, Value> = (a, b) => Collection.defaultSort(a, b)): this {
		const entries = [...this.entries()];
	 ...
	 * ```
	 */
	public intersection(other: ReadonlyCollection<Key, unknown>): Collection<Key, Value> {
	 ...
	 * ```
	 */
	public difference(other: ReadonlyCollection<Key, unknown>): Collection<Key, Value> {
		const coll = new this.constructor[Symbol.species]<Key, Value>();
	...
	public merge<OtherValue, ResultValue>(
		...
		const keys = new Set([...other.keys(), ...this.keys()]);
	...
	public static combineEntries<Key, Value>(
		...
		const coll: Collection<Key, Value> = new Collection();
		for (const [key, value] of entries)
			if (coll.has(key))

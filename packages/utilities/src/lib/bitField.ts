const FlagEntriesSymbol: unique symbol = Symbol('@flowr/bitfield.flags.entries');
...
export class BitField<Flags extends Record<string, number> | Record<string, bigint>> {
	...
	public constructor(flags: Readonly<Flags>) {
		...
		const entries = Object.entries(flags) as Array<[string, Flags[keyof Flags]]>;
		if (entries.length === 0)
	 ...
	 * // 0b1111
	 * ```
	 * @seealso {@linkplain https://en.wikipedia.org/wiki/Union_(set_theory)}
	 */
	public union(...fields: ReadonlyArray<ValueResolvable<this>>): ValueType<this> {
	 ...
	 * // 0b0100
	 * ```
	 * @seealso {@linkplain https://en.wikipedia.org/wiki/Intersection_(set_theory)}
	 */
	public intersection(bitfield: ValueResolvable<this>, ...fields: ReadonlyArray<ValueResolvable<this>>): ValueType<this> {
	 ...
	 * // 0b1001
	 * ```
	 * @seealso {@linkplain https://en.wikipedia.org/wiki/Difference_(set_theory)}
	 */
	 ...
	 * // 0b0110
	 * ```
	 * @seealso {@linkplain https://en.wikipedia.org/wiki/Symmetric_difference}
	 */
	 ...
	 * ```
	 */
	public toArray(field: ValueResolvable<this>): Array<keyof Flags> {
	...
	public *toValues(field: ValueResolvable<this>): IterableIterator<ValueType<this>> {
		...
		for (const [_, bit] of this[FlagEntriesSymbol])
			if ((bits & bit) === bit)
				yield bit as unknown as ValueType<this>;
	}
...
export type PrimitiveType<Type> = Type extends number ? number : bigint;
export type MaybeArray<Type> = Type | readonly Type[];

/**
 * Resolves the type of the values the specified {@link BitField} takes.
 * @typeparam A {@link BitField} instance type.
 */
export type ValueType<Type> = Type extends BitField<infer Flags> ? PrimitiveType<Flags[keyof Flags]> : never;

/**
 * Resolves the possible types accepted by the specified {@link BitField}.
 * @typeparam A {@link BitField} instance type.
 */
export type ValueResolvable<Type> = Type extends BitField<infer Flags> ? MaybeArray<keyof Flags | PrimitiveType<Flags[keyof Flags]>> : never;

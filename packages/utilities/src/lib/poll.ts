 ...
 */
export async function poll<Return>(
	cb: (signal: AbortSignal | undefined) => Awaitable<Return>,
	cbCondition: (value: Awaited<Return>, signal: AbortSignal | undefined) => Awaitable<boolean>,
	options: PollOptions = {},
): Promise<Awaitable<Return>> {
	...
	for (let retries = 0; retries < maximumRetries && !(await cbCondition(result, signal)); retries++) {
		...
		if (waitBetweenRetries > 0) {
			if (options.verbose)
				// eslint-disable-next-line no-console -- info logging (move to node utils?)

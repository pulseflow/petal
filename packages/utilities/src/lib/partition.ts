 ...
 */
export function partition<ArrayType>(array: ArrayType[], predicate: (value: ArrayType, index: number) => boolean): ArrayType[][] {
	if (!Array.isArray(array))
		throw new TypeError('entries must be an array.');
	if (!isFunction(predicate))
		throw new TypeError('predicate must be an function that returns a boolean value.');

	const partitionOne: ArrayType[] = [];
	const partitionTwo: ArrayType[] = [];

...
const tokenResolvers: Map<string, TokenResolver> = new Map([
	// Dates
...
export class Timestamp {
	 ...
	 *
	 */
	public template: TimestampTemplateEntry[];
	...
	private static parse(pattern: string): TimestampTemplateEntry[] {
		...
		for (let i = 0; i < pattern.length; i++) {
			...
			if (typeof tokenMax === 'number') {
				...
				template.push({ content: null, type: current });
			}
			else if (currentChar === '[') {
				while (i + 1 < pattern.length && pattern[i + 1] !== ']') current += pattern[++i];
				i++;
				template.push({ content: current || '[', type: 'literal' });
			}
			else {
				current += currentChar;

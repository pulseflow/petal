...
describe('converter', () => {
	describe('verify support for all combinations', () => {
		it('should support primary-default to primary', () => {
			const expected = 0.001;
			const actual = convert(1, 'g', 'kg');
			expect(actual).toBe(expected);
		});

		it('should support primary to primary-default', () => {
			const expected = 1000;
			const actual = convert(1, 'kg', 'g');
			expect(actual).toBe(expected);
		});

		it('should support primary to primary', () => {
			const expected = 0.001;
			const actual = convert(1, 'kg', 'mt');
			expect(actual).toBe(expected);
		});

		it('should support secondary to secondary', () => {
			const expected = 0.00003125;
			const actual = convert(1, 'oz', 't');
			expect(actual).toBe(expected);
		});

		it('should support secondary-default to secondary', () => {
			const expected = 16;
			const actual = convert(1, 'lb', 'oz');
			expect(actual).toBe(expected);
		});

		it('should support secondary to secondary-default', () => {
			const expected = 0.0625;
			const actual = convert(1, 'oz', 'lb');
			expect(actual).toBe(expected);
		});

		it('should support primary-default to secondary-default', () => {
			const expected = 0.00220462;
			const actual = convert(1, 'g', 'lb');
			expect(actual).toBe(expected);
		});

		it('should support secondary-default to primary-default', () => {
			const expected = 453.592;
			const actual = convert(1, 'lb', 'g');
			expect(actual).toBe(expected);
		});

		it('should support primary to secondary-default', () => {
			const expected = 2.20462442;
			const actual = convert(1, 'kg', 'lb');
			expect(actual).toBe(expected);
		});

		it('should support secondary to primary-default', () => {
			const expected = 28.3495;
			const actual = convert(1, 'oz', 'g');
			expect(actual).toBe(expected);
		});

		it('should support primary to secondary', () => {
			const expected = 35.27399072;
			const actual = convert(1, 'kg', 'oz');
			expect(actual).toBe(expected);
		});

		it('should support secondary to primary', () => {
	...
	describe('verify transform ratio works', () => {
		it('should support celcius to fahrenheit', () => {
			const expected = 33.8;
			const actual = convert(1, 'c', 'f');
			expect(actual).toBe(expected);
		});

		it('should support fahrenheit to celcius', () => {
		...
		describe('verify value shift works', () => {
			it('should support celcius to kelvin', () => {
				const expected = 274.15;
				const actual = convert(1, 'c', 'k');
				expect(actual).toBe(expected);
			});

			it('should support celcius to rankine', () => {
				const expected = 493.47;
				const actual = convert(1, 'c', 'r');
				expect(actual).toBe(expected);
			});

			it('should support fahrenheit to kelvin', () => {
				const expected = 255.92777778;
				const actual = convert(1, 'f', 'k');
				expect(actual).toBe(expected);
			});

			it('should support fahrenheit to rankine', () => {
	...
	describe('verify support for options', () => {
		it('modifying decimals with bit to kilobit', () => {
			const expected = 0.00098;
			const actual = convert(1, 'b', 'kb', { precision: 5 });
			expect(actual).toBe(expected);
		});

		it('modifying decimals with herz to radians per second', () => {
			const expected = 6.28;

...
function expectInvalidFunctionError(cb: () => unknown, val?: never): void {
	expect(() => {
		cb();
...
describe('sort() tests', () => {
	...
	describe('defaultSort', () => {
		it('stays the same if it is already sorted (single)', () => {
			const coll = createTestCollection();
			expect(coll.sort()).toStrictEqual(coll);
		});

		it('stays the same if it is already sorted (multiple)', () => {
		...
		describe('returns correct sort order for test values', () => {
			const testDefaultSortOrder = (firstValue: any, secondValue: any, result: number) => {
				expect(Collection.defaultSort(firstValue, secondValue)).toStrictEqual(result);
				expect(Collection.defaultSort(secondValue, firstValue)).toStrictEqual(result ? result * -1 : 0);
			};

			it('correctly evaluates sort order of undefined', () => {
				testDefaultSortOrder(undefined, undefined, 0);
				testDefaultSortOrder(0, undefined, -1);
			});

			it('correctly evaluates numeric values stringwise', () => {
			...
			it('evaluates object literals as equal', () => {

...
export const typescript = async (ctx: TypescriptContext) => {
	...
	if (ts === undefined) {
		...
		if (
			!['strict', 'strictest', 'relaxed', 'default', 'base', 'esm'].includes(ts)
		) {
			if (!ctx.dryRun) await rm(ctx.cwd, { recursive: true, force: true });
			error(
				'error',
	...
	'tsconfig.json': async (file: string, options: { value: string }) => {
		try {
			...
			if (templateTSConfig && typeof templateTSConfig === 'object') {
				...
				throw new Error(`there was an error applying typescript settings.`);
			}
		} catch (err) {

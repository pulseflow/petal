...
async function main() {
	for await (const file of findFilesRecursivelyStringEndsWith(resolve(import.meta.dirname, `../../${PACKAGE_NAME}/src/lib`), '.ts')) {
		...
		if (PACKAGE_NAME === 'utilities' && name === 'debounce')
			...
			EXPORT_MAP.set(`./${name.charAt(0).toLowerCase() + name.slice(1)}`, `./src/lib/${name}.ts`);
		else
	...
	if (oldJsrJSON === newJsrJSON) {
		console.log(green(`The jsr.json file for ${bold(PACKAGE_NAME)} is up to date!`));
		process.exit(0);
	}

	if (CHECK_MODE) {
		console.error(red(`The jsr.json file for ${bold(PACKAGE_NAME)} is not up to date! Run ${yellow(bold('pnpm build'))} to update it.`));
		process.exit(1);
	}

	await writeFile(PACKAGE_PATH, newJsrJSON);
	console.log(green(`The jsr.json file for ${bold(PACKAGE_NAME)} is updated successfully!`));
}
